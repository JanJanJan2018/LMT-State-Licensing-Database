...
nanny <- read.csv('./Indeed 10/jobListings_nanny.csv', header=TRUE,
                            na.strings=c('',' ','NA'), sep=',')
sd <- as.data.frame(t(c('sd',rep(NA,8))))
colnames(sd) <- colnames(nanny)
nanny2 <- rbind(nanny[1:40,],sd,nanny[41:49,]) #sd NA
nanny <- nanny2
...
```{r}
remote <- read.csv('./Indeed 10/jobListings_remote.csv', header=TRUE,
                            na.strings=c('',' ','NA'), sep=',')
remote$stateName <- states
remote <- remote[,c(1,10,2:9)]
head(remote)
...
warehouse worker:
```{r}
warehouseWorker <- read.csv('./Indeed 10/jobListings_warehouse.csv', header=TRUE,
                            na.strings=c('',' ','NA'), sep=',')
...
personal assistant:
```{r}
personal_assistant <- read.csv('./Indeed 10/jobListings_personal assistant.csv', header=TRUE,
                            na.strings=c('',' ','NA'), sep=',')
personal_assistant$stateName <- states
personal_assistant <- personal_assistant[,c(1,10,2:9)]
head(personal_assistant)
...
colnames(nanny2)[2:3] <- paste('nanny',colnames(nanny2)[2:3],
                                         sep='_')
nanny2$nanny_avgHourly <- as.numeric(nanny2$nanny_avgHourly)
...
```{r}
remote2 <- remote[,c(2,3,9)]
colnames(remote2)[2:3] <- paste('remote',colnames(remote2)[2:3],
                                          sep='_')
remote2$remote_avgAnnualSalary <- remote2$remote_avgHourly*52*40
head(remote2)
```
...
```{r}
personal_assistant2 <- personal_assistant[,c(2,3,10)]
colnames(personal_assistant2)[2:3] <- paste('personal_assistant',colnames(personal_assistant2)[2:3],sep='_')
personal_assistant2$personal_assistant_avgHourly <- (personal_assistant2$personal_assistant_avgAnualSalary/52)/40
personal_assistant2 <- personal_assistant2[,c(1,2,4,3)]
head(personal_assistant2)
```
...
slr <- merge(slr, remote2, by.x='state', by.y='stateName')
...
slr <- merge(slr, personal_assistant2, by.x='state', by.y='stateName')
...
Lets now write this data table out to csv. The server job was replaced with remote as it is insurance and sales mostly and quarantine took out many server jobs. The teacher job was replaced with personal assistant same logical reason. There was an error with South Dakota in the original nanny table, so it was fixed.
```{r}
...
highLivingCost
```
There are 20 states that are listed as possible high cost of living states because we are making the assumption that those states with higher than average pay for LMTs and higher than average job listings for LMT are in high cost of living states. We can verify this by looking at the two bedroom homes for sale in all states and get the average to compare to these 20 states.The July Zillow report hasn't been updated and consequently is using June 2020 data as of Aug 8, 2020.
```{r}
...
expensiveHomesList <- e[expensive]
expensiveHomesList
```
...
```{r}
expensiveDemog <- subset(slr, slr$state %in% expensiveHomesList)
...
```{r}
dataAnalystEntry <- read.csv('./Indeed 10/jobListings_data analyst entry level.csv', sep=',',
                    header=TRUE, na.strings=c('',' ','NA'))
...
#nd and wy are missing, 
ch1 <- chiropractor[1:27,]
ch2 <- chiropractor[28:48,]
ch3 <- chiropractor[1,]
ch3$state <- 'nd'
ch3[,2:9] <- NA
ch4 <- ch3
...
chiropractor <- rbind(ch1,ch3,ch2,ch4)
...
wy <- ak
wy$state <- 'wy'
p1 <- pilatesInstructor[1:26,]
p2 <- pilatesInstructor[27:47,]

pilatesInstructor <- rbind(ak,p1,nd,p2,wy)

pilatesInstructor$stateName <- states[1:50]
...
listedJobsAll2 <- listedJobsAll[,-12]#added personal Assistant 2x bw weeks missing
#nurses, now realized
listedJobsAll2
```
...
```{r}
wellnessJobs <- listedJobsAll2[,c(1,2,5,15:20)]
wellnessSalary <- avgSalaryAll2[,c(1,2,5,15:20)]
colnames(wellnessJobs)[2]<- 'massageTherapist'
wellnessJobs
```
...
```{r}
spaJobs <- listedJobsAll2[,c(1,2,17,18)]
spaSalary <- avgSalaryAll2[,c(1,2,17,18)]
colnames(spaJobs)[2] <- 'massageTherapist'
spaJobs
```
...
GA_spa2 <- gather(GA_spa, key='jobTitle',value='jobsListed',2:4)
GA_spaSalary2 <- gather(GA_spaSalary, key='jobTitle', value='annualSalary',2:4)
...
comparing_TN_CA <- slr5[,c(1,11,12,17,18,19,23,33,35,37:45,55,57,58,60,
                           64,66,67,69,70,72,76,78,82,84,100:115)]
...
```{r}
CA_jobsAvailable <- CA[,c(1,8,19,21,23,25,27,29,31,33)]
TN_jobsAvailable <- TN[,c(1,8,19,21,23,25,27,29,31,33)]
