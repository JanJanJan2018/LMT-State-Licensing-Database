...
```{r}
for (i in Table10$functionInput[22:25]){
  writeYellowPageBusinessTypes(i)
...
```{r}
for (i in Table10$functionInput[151:377]){
  writeYellowPageBusinessTypes(i)
...
```{r}
for (i in Table10$functionInput[378:500]){
  writeYellowPageBusinessTypes(i)
  ...
  colnames(BizCities) <- header$x
  colnames(BizCities)[3] <- paste(BUSINESS,colnames(BizCities)[3],sep='_')
  write.csv(BizCities, 'BizCities.csv', row.names=FALSE)
