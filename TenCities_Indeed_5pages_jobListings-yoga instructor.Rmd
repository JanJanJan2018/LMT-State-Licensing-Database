...
```{r}
for (i in Table10$functionInput[1:117]){
...
```{r}
for (i in Table10$functionInput[118:500]){
  writeIndeedJobData5pages(i)
  pausedDT <- read.csv(paste(busDir,'/allCities-',business,'.csv',
                            sep=''),
                            sep=",", header=FALSE, na.strings=c('',' ','NA'))
...
TN missing in yoga df.
```{r}

# us <- as.character(paste(tolower(unique(Table10$stateAbbreviations))))
# sr <- as.character(paste(statesRates$state))
# u <- sort(us)
# s <- sort(sr)
# u
# s
# s %in% u #it says they're all there, but sd isn't in yoga df

# grep('sd',statesRates$state)
# statesRates$state
# sort(statesRates$state)
# sc <- grep('sc',statesRates$state)
# s <- statesRates[1:sc,]
# s2 <- statesRates[41:49,]
# sd <- s[1,]
# sd$state <- 'sd'
# sd[,2:9] <- NA
# 
# sr <- rbind(s,sd,s2)
# sr
# write.csv(sr,paste('jobListings_',business,'.csv',sep='')
#            , row.names=FALSE)
```
